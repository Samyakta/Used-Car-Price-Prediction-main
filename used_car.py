...
dataframe_train = pd.read_csv('train-data.csv') #read csv file 

train_data = dataframe_train.copy()

for i in range(0, len(train_data)):
    if train_data['Power'][i] == 'null bhp':
        train_data['Power'][i] = np.nan #not a number--fill data as nan if no value is present 

#nan value if mileage value is 0 --drop mileage
for i in range(0, len(train_data)):
    if train_data['Mileage'][i] == '0.0 kmpl' or train_data['Mileage'][i] == '0.0 km/kg':
        train_data['Mileage'][i] = np.nan 

#nan value if Engine value is 0--drop engine 
for i in range(0, len(train_data)):
    if train_data['Engine'][i] == 'null CC' or train_data['Engine'][i] == '0 CC':
        train_data['Engine'][i] = np.nan

#dropping values of columns: New price and Unnamed   --> dropna-> drop all missing values   
train_data.drop(['New_Price'], axis=1, inplace=True)
train_data.drop(['Unnamed: 0'], axis=1, inplace=True)
train_data.dropna(inplace = True)

#put index number to the given dataset
train_data.reset_index(inplace = True)

#drop index column axis=1 means drop column
...
#spliting the brand of cars if it contains 2 string values
for i in range(0, 5844):
    k = train_data['Name'][i].split()
    brand.append(k[0].upper())
    
#create and store values in array
